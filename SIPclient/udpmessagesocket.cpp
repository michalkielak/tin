...
int UDPMessageSocket::send( const char *sendbuffer, unsigned int length )
{
	...
	si_other.sin_port = htons(8060);
	si_other.sin_addr.s_addr = inet_addr("127.0.0.1");


		//int soal=intype==Inet6?siz, eof(struct sockaddr_in6):intype==Inet4?sizeof(struct sockaddr_in):0;
while(1){
	if ( (numbytes = sendto( socketfd, sendbuffer, length, 0,(struct sockaddr*)&si_other, sizeof(si_other)) ) == -1 ) {
		if( !didcomplain ) {
