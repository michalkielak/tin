...
void SRTPWrapper::protect(unsigned char* rtp_h, int* length){
	...
	if(!sessionActive){
		if(settings.readEntry(KStatics::dBase + "SRTP/Mode", "disabled") == "PSK"){
			...
			cout << "Key exchange Mode not supported: " << 	settings.readEntry(KStatics::dBase + "SRTP/Mode", "disabled") << endl;
			return;
		}
	}
...
unsigned char* SRTPWrapper::readKey(){
	QSettings settings;
...
void SRTPWrapper::printPolicyInfo(){
	...
	cout << "Key Exchange Mode: " << settings.readEntry(KStatics::dBase + "SRTP/Mode", "disabled");
