...
//#PR  Contact
	if( contactlist.getListLength() > 0 ) {
		...
		if( meth == Sip::REGISTER ) {
			...
			headers += ";methods=\"INVITE, MESSAGE, INFO, SUBSCRIBE, OPTIONS, BYE, CANCEL, NOTIFY, ACK, REFER, PRACK\"";
		}
		headers += "\r\n";
	}
...
QString SipMessage::message( void )
{
	...
	msg = startLine();
	if (KStatics::debugLevel == 2) cout <<  msg.latin1() << "<<<<<\n";
	msg += messageHeaders() + "\r\n";
	if ( haveBody() ) {
		msg += messageBody();
	}
...
void SipMessage::parseHeaders( const QString &inbuf )
{
	...
	serviceRoute = "";
	for (;;) {
		if( !HeaderParser::parse( inbuf.utf8().mid( lastheader ).data(),
			inbuf.utf8().mid( lastheader ).length(), &headerend, &bodystart, &bodylength ) ) {
			break;
		}
		curheader = inbuf.utf8().mid( lastheader, headerend ).simplifyWhiteSpace();
		curbody = inbuf.mid( lastheader + bodystart, bodylength ).simplifyWhiteSpace();
		id = SipHeader::matchHeader( curheader.utf8() );

		if ( id == SipHeader::Via ) {
			vialist.parseVia( curbody );
		} else if( id == SipHeader::Min_Expires ) {
			minExpires = curbody.simplifyWhiteSpace();
			KStatics::minExT = minExpires;
		} else if( id == SipHeader::Service_Route ) {
			serviceRoute = curbody.simplifyWhiteSpace();
			insertHeader( id, curbody );
		} else if (id == SipHeader::SIP_ETag ) {
			SipETag = curbody.simplifyWhiteSpace();
		} else if (id == SipHeader::Refer_Sub ) {
			RefSub = curbody.simplifyWhiteSpace();
		...
		if ( lastheader >= inbuf.utf8().length() ) {
			break;
		}
	}
}
