...
void SipTransaction::getpidfxml( const QString m) {
	...
	if (doc.setContent(m, true)) {
			...
			if (!qde.isNull()) {
				...
				if (nl2.length() > 0) {
					...
					if(gok.localName()  == "activity") { // we have <activity>text</activity>
					    ...
					    if(KStatics::debugLevel>= 2) cout << "toElement " << gok.toElement().text() << endl;
					    
					} else {
					    parent->setPresenceStatus( gok.localName());
					    if(KStatics::debugLevel>= 2) cout << " localName " << gok.localName() << endl;
					}
					parent->setBasePresenceStatus( bc );
					return;
				}
		...
		cout << "=====SipTransaction::getpidfxml minimalist\n";
		if (!bc) {	
